...
class SurchargeGrid extends Value
{
    ...
    {
        // The grid POSTs its value as a nested array: [days => [field => value]]
        $gridValues = $this->getValue();
        if (!is_array($gridValues)) {
            return parent::afterSave();
        }

        // Inherit flags arrive separately: [days => [field => '1']]
        $fieldData = $this->getData('fieldset_data');
        $inheritData = [];
        if (is_array($fieldData) && isset($fieldData['surcharge_grid']['inherit'])) {
            $inheritData = $fieldData['surcharge_grid']['inherit'];
        }
        // Also check the groups structure for inherit flags
        $groups = $this->getData('groups');
        if (is_array($groups)
            && isset($groups['payment_terms']['fields']['surcharge_grid']['inherit'])
        ) {
